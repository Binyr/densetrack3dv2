...
class EvaluationPredictor(torch.nn.Module):
    ...
    def forward_sparse(
        ...
        if use_2d_only:
            ...
            sparse_predictions, dense_predictions, coords_predicted_lis = self.model(
                video=video,
                videodepth=videodepth,
                sparse_queries=queries,
                iters=self.n_iters,
        ...
        traj_e[:, :, :, 0] *= (W - 1) / float(self.interp_shape[1] - 1)
        traj_e[:, :, :, 1] *= (H - 1) / float(self.interp_shape[0] - 1)
        for i in range(len(coords_predicted_lis)):
            coords_predicted_lis[i][..., 0] *= (W - 1) / float(self.interp_shape[1] - 1)
            coords_predicted_lis[i][..., 1] *= (H - 1) / float(self.interp_shape[0] - 1)

        return traj_e, traj_d_e, vis_e, coords_predicted_lis
