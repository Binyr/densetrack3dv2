...
from densetrack3d.utils.PO import geom, misc, improc
# from densetrack3d.utils.PO import utils.basic
# import utils.misc
# import utils.improc
import glob
import cv2
...
class PointOdysseyDataset(torch.utils.data.Dataset):
            ...
            if os.path.isfile(annotations_path):
                for stride in strides:
                    ...
                    for ii in range(0,len(os.listdir(rgb_path))-self.S*stride+1, clip_step):
                        ...
                        #
                        seq_name = seq.split('/')[-1]
                        self.sequences_name.append(f"{seq_name}_" + "_".join([str(x) for x in full_idx]))
                        # break
                    if self.verbose:
                        sys.stdout.write('.')
                        sys.stdout.flush()
    ...
    def getitem_helper(self, index):
        ...
        # binyanrui - filter out traj that contains invalid point
        # valid_traj = (1 - valids).sum(axis=0) == 0
        # trajs = trajs[:, valid_traj]
        # visibs = visibs[:, valid_traj]
        # valids = valids[:, valid_traj]
        # trajs_world = trajs_world[:, valid_traj]
        # get trajectory d
        # import pdb
        # pdb.set_trace()
        trajs_cam = geom.apply_4x4_py(cams_T_world, trajs_world)
        # trajs_pix = utils.geom.apply_pix_T_cam_py(pix_T_cams, trajs_cam)
        trajs_d = trajs_cam[:, :, 2:3]
        ...
        inds = misc.farthest_point_sample_py(xym, self.N)
        trajs = trajs[:,inds]
        visibs = visibs[:,inds]
        valids = valids[:,inds]
        trajs_d = trajs_d[:, inds]
        ...
        # trajs_d_full = np.zeros((self.S, self.N, 1)).astype(np.float32)
        # trajs_d_full[:,:N_] = trajs_d[:,inds]
        trajs_full = trajs
        visibs_full = visibs
        valids_full = valids
        trajs_d_full = trajs_d
        rgbs = torch.from_numpy(np.stack(rgbs, 0)).permute(0,3,1,2).byte() # S,C,H,W
        depths = torch.from_numpy(np.stack(depths, 0)).unsqueeze(1).float() # S,1,H,W
    ...
    def just_crop(self, rgbs, masks, edges, trajs, depths, pix_T_cams):
        ...
        depths = [d[y0:y0+H_new, x0:x0+W_new] for d in depths]
    ...
    def just_resize(self, rgbs, masks, edges, trajs, depths, pix_T_cams):
        ...
        sy_ = H_new / H
        rgbs = [cv2.resize(rgb, (W_new, H_new), interpolation=cv2.INTER_LINEAR) for rgb in rgbs]
        depths = [cv2.resize(rgb, (W_new, H_new), interpolation=cv2.INTER_NEAREST) for rgb in depths]
        masks = [cv2.resize(mask, (W_new, H_new), interpolation=cv2.INTER_NEAREST) for mask in masks]
        edges = [cv2.resize(edge, (W_new, H_new), interpolation=cv2.INTER_NEAREST) for edge in edges]
...
def from_sample_to_DeltaData(sample):
    """
    sample = {
            'rgbs': rgbs,
            'masks': masks,
    ...
    return DeltaData(
            video=sample["rgbs"].float(),
            videodepth=depths.float(),
            videodepth_pred=torch.zeros_like(depths),
            depth_init=depth_init,
            trajectory=sample["trajs"],
            trajectory_d=sample["trajs_d"],
            visibility=sample["visibs"],
            valid=sample["valids"],
