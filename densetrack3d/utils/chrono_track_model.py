...
REPO_DIR = "/mnt/shared-storage-user/binyanrui/Projects/dinov2"
DINO_DIR = "/mnt/shared-storage-user/idc2-shared/binyanrui/pretrained_models/dino/v2/"
...
class LocoTrack(nn.Module):
  ...
  def __init__(
    ...
    pretrained_model_name_or_path = f"{DINO_DIR}{backbone_name}_pretrain.pth"
    # self.dino = torch.hub.load(repo_or_dir="facebookresearch/dinov2", model=backbone_name)
    self.dino = torch.hub.load(REPO_DIR, backbone_name, source='local', pretrained=False)
  ...
  def forward_dino(
    ...
    """
    IMAGENET_DEFAULT_MEAN = torch.tensor((0.485, 0.456, 0.406), device=video.device).view(1,1,3,1,1)
    IMAGENET_DEFAULT_STD = torch.tensor((0.229, 0.224, 0.225), device=video.device).view(1,1,3,1,1)
    # import pdb
    # pdb.set_trace()
    B, T = video.shape[:2]
    video = (video + 1) / 2
    ...
    for block_idx, block in enumerate(self.dino.blocks[:-1]):
      def hook_fn(module, input, output, block_index=block_idx):
        # print(f"{block_idx}",output.shape)
    ...
    video_feat = rearrange(video_feat[-1], '(b t) (h w) c -> b t c h w', b=B, h=H_f, w=W_f)
    ...
    video_feat = video_feat / torch.sqrt(
      torch.maximum(
          torch.sum(torch.square(video_feat), axis=2, keepdims=True),
