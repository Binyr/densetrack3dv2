...
from densetrack3d.utils.PO import basic
from densetrack3d.utils.PO.basic import print_
...
def get_box2d_from_mask(mask, normalize=False):
    ...
    B, C, H, W = mask.shape
    assert(C==1)
    xy = basic.gridcloud2d(B, H, W, norm=False, device=mask.device) # B, H*W, 2
...
def depth2pointcloud(z, pix_T_cam):
    ...
    device = z.device
    y, x = basic.meshgrid2d(B, H, W, device=device)
